...
const getDownloadUrl = async () => {
	try {
		...
		handleError(
			'Download preparation failed: ' + (error instanceof Error ? error.message : 'Unknown error')
		)
	}
}
```
...
```jsx
<Button variant="primary" size="lg" onClick={handleDownload} disabled={!downloadUrl}>
